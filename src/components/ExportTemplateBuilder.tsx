  ...
  return (
    <div
      ...
      className={`flex-1 min-h-[32px] flex flex-wrap gap-1 p-1.5 rounded border border-dashed transition-colors ${
        isOver ? 'border-[#d4600a] bg-[#BF5700]/10' : 'border-[#3a3a46]/50 bg-[#1f1f24]/30'
      }`}
    >
      {children}
    </div>
...
function SortableColumnCard({
  ...
  return (
    <div
      ref={setNodeRef}
      style={style}
      className="flex items-start gap-2 bg-[#232329]/60 rounded-lg border border-[#3a3a46]/50 p-2.5 group"
    >
      {/* Drag handle */}
      <button
        type="button"
        className="cursor-grab active:cursor-grabbing text-[#4e4a56] hover:text-[#8a8680] touch-none mt-1 shrink-0"
        {...attributes}
        {...listeners}
      >
          ...
          {column.locked ? (
            ...
            <>
              <input
                ...
                className="flex-1 bg-[#1f1f24] border border-[#3a3a46] rounded px-2 py-0.5 text-xs text-[#ede9e3] focus:border-[#BF5700] focus:outline-none min-w-0"
              ...
              {column.customName && (
                <button
                  ...
                  className="p-0.5 text-[#4e4a56] hover:text-[#8a8680] shrink-0"
        ...
        {!column.locked && (
          <ColumnDropTarget columnId={column.id}>
            {column.fieldKeys.length === 0 ? (
              <span className="text-[10px] text-[#4e4a56] italic px-1">Drop fields here</span>
            ) : (
              column.fieldKeys.map((fk) => (
                <span
                  key={fk}
                  className="inline-flex items-center gap-1 px-2 py-0.5 rounded bg-[#BF5700]/30 text-[#BF5700] text-[10px] font-medium"
                >
      ...
      {!column.locked && (
        <button
          ...
          className="p-1 text-[#4e4a56] hover:text-red-400 opacity-0 group-hover:opacity-100 transition-opacity shrink-0 mt-0.5"
          title="Remove column"
        >
...
function DraggablePoolField({ fieldKey, label }: { fieldKey: string; label: string }) {
  ...
  return (
    <div
      ref={setNodeRef}
      style={style}
      className="flex items-center gap-1.5 px-2.5 py-1.5 bg-[#232329]/50 rounded border border-[#3a3a46]/40 text-xs text-[#8a8680] cursor-grab active:cursor-grabbing hover:bg-[#232329] hover:border-[#4a4a56] transition-colors select-none"
      {...attributes}
      {...listeners}
    >
      <GripVertical className="w-3 h-3 text-[#4e4a56] shrink-0" />
      {label}
    </div>
  );
}
...
export function ExportTemplateBuilder({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <div className="bg-[#1f1f24] border border-[#2c2c36] rounded-xl shadow-2xl w-full max-w-5xl max-h-[90vh] flex flex-col">
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#2c2c36] shrink-0">
          <h2 className="text-lg font-semibold text-[#ede9e3]">Export XLSX — Template Builder</h2>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="flex items-center gap-3 px-6 py-3 border-b border-[#2c2c36]/50 shrink-0 bg-[#1f1f24]/50">
          ...
          <div className="relative flex-1 max-w-xs">
            <select
              ...
              className="w-full bg-[#232329] border border-[#3a3a46] rounded-md px-3 py-1.5 text-xs text-[#ede9e3] appearance-none pr-8 focus:border-[#BF5700] focus:outline-none"
            >
          ...
          <input
            ...
            className="flex-1 max-w-xs bg-[#232329] border border-[#3a3a46] rounded-md px-3 py-1.5 text-xs text-[#ede9e3] focus:border-[#BF5700] focus:outline-none"
          />
          <button
            type="button"
            onClick={handleSaveTemplate}
            className="flex items-center gap-1 px-3 py-1.5 text-xs bg-[#BF5700] text-white rounded-md hover:bg-[#BF5700] transition-colors shrink-0"
          >
          ...
          {selectedTemplateId && (
            <button
              ...
              className="p-1.5 text-[#4e4a56] hover:text-red-400 hover:bg-[#232329] rounded-md transition-colors shrink-0"
        ...
        <div className="flex-1 min-h-0 flex overflow-hidden">
          ...
          >
            {/* Left: Field Pool */}
            <div className="w-56 border-r border-[#2c2c36] flex flex-col shrink-0">
              <div className="px-3 pt-3 pb-2 border-b border-[#2c2c36]/50">
                <h3 className="text-xs font-semibold text-[#8a8680] uppercase tracking-wider mb-2">Field Pool</h3>
                <input
                  type="text"
                  value={poolSearch}
                  onChange={(e) => setPoolSearch(e.target.value)}
                  placeholder="Search fields…"
                  className="w-full bg-[#232329]/50 border border-[#3a3a46]/50 rounded px-2 py-1 text-xs text-[#8a8680] focus:border-[#BF5700] focus:outline-none"
                />
              </div>
              <div className="flex-1 overflow-y-auto px-3 py-2 space-y-1 annotation-scroll">
                ...
                {poolFields.length === 0 && (
                  <p className="text-[10px] text-[#4e4a56] italic text-center py-4">No matching fields</p>
                )}
              </div>
            </div>
            ...
            <div className="flex-1 flex flex-col min-h-0">
              <div className="flex items-center gap-2 px-4 pt-3 pb-2 border-b border-[#2c2c36]/50">
                <h3 className="text-xs font-semibold text-[#8a8680] uppercase tracking-wider flex-1">
                  Columns ({columns.length})
                </h3>
                <button
                  type="button"
                  onClick={addColumn}
                  className="flex items-center gap-1 px-2.5 py-1 text-[10px] font-medium bg-[#232329] text-[#8a8680] rounded hover:bg-[#2e2e38] transition-colors"
                >
              ...
              <div className="flex-1 overflow-y-auto px-4 py-3 space-y-2 annotation-scroll">
                ...
                {userColumnCount === 0 && (
                  <div className="text-center py-8 text-[#4e4a56]">
              ...
              <div className="border-t border-[#2c2c36]/50 px-4 py-3 space-y-3 shrink-0">
                {/* Cue type colour overrides */}
                <div>
                  <h4 className="text-[10px] font-semibold text-[#8a8680] uppercase tracking-wider mb-2">Cue Type Colours</h4>
                      ...
                      return (
                        <label key={ct} className="flex items-center gap-1.5 cursor-pointer">
                          <input
                            ...
                            className="w-5 h-5 rounded border border-[#3a3a46] cursor-pointer"
                          />
                          <span className="text-[10px] text-[#8a8680]">{ct}</span>
                        </label>
                      );
                    })}
                  </div>
                ...
                <label className="flex items-center gap-2 cursor-pointer select-none">
                  ...
                  />
                  <span className="text-xs text-[#8a8680]">Include skipped cues</span>
                  <span className="text-[10px] text-[#4e4a56]">(italicised in export)</span>
                </label>
              </div>
            </div>
            ...
            <DragOverlay>
              {activePoolField && (
                <div className="px-2.5 py-1.5 bg-[#BF5700]/80 rounded border border-[#d4600a] text-xs text-white font-medium shadow-lg">
        ...
        {/* Footer */}
        <div className="flex items-center justify-between px-6 py-3 border-t border-[#2c2c36] shrink-0">
          ...
          <div className="flex items-center gap-2">
            <button
              type="button"
              onClick={onClose}
              className="px-4 py-2 text-xs bg-[#232329] text-[#8a8680] rounded-md hover:bg-[#2e2e38] transition-colors"
            >
              Cancel
            </button>
            <button
